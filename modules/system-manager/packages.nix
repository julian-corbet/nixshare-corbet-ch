...
# reconciler. Once those packages exist, nixshare makes its selected packages
# explicit, so a package that happened to arrive as another package's
...
let
  ...
  packages = cfg.archPackages ++ cfg.aurPackages;
  ownership = pkgs.writeShellScript "nixshare-package-ownership" ''
    set -euo pipefail
    /usr/bin/pacman -D --asexplicit ${lib.escapeShellArgs packages}
  '';
in
{
  options.nixshare.systemManager.packageReconcilerUnit = lib.mkOption {
    ...
    description = ''
      Optional systemd unit that installs `nixshare.archPackages` and
      `nixshare.aurPackages`. When set, nixshare's ownership unit requires and
      runs after it before marking the selected packages explicit. The default
      keeps nixshare independent of a particular Arch package reconciler.
    '';
  };

  config = lib.mkIf (cfg.enable && packages != [ ]) {
